...
// 후원 이벤트 크기 기준 — 누적 총합이 그 숫자를 지나칠 때가 아님) 무작위 가격 보정 효과가
// 발동합니다. 같은 방송에서 정확히 그 개수짜리 후원이 여러 번 들어오면 그때마다 매번 다시
// 발동합니다(반복 가능, 1회성 아님). 효과는 누적되며(최대 ±50%), 이후 모든 가격 계산에
// 계속 적용됩니다.
//
// 예) 후원 30개(누적 30개) → 발동 / 후원 20개(누적 50개) → 미발동(20은 기준이 아님) /
//     후원 31개(누적 81개) → 발동 / 후원 300개(누적 381개) → 발동 / 후원 100개(누적 481개)
//     → 미발동 / 후원 301개(누적 782개) → 발동. (사용자 확정 사항, 2026-09-11)
//
...
// 트리거 표(사용자 확정 사항, 2026-09-11 — "한 번의 후원 크기"가 정확히 이 숫자일 때만 발동):
...
// n이 정확히 30/31/300/301이면 그 즉시 무작위 보정 효과가 발동합니다(누적 총합이 그 숫자를
